...
class MessagingServiceImplTest {
    ...
    void sendMessage_persistsAndBroadcasts_whenSenderIsParticipant() {
        when(userRepo.findByPublicId(alice.getPublicId()))
                .thenReturn(Optional.of(alice));
        when(conversationRepo.findByPublicId(conv.getPublicId()))
                .thenReturn(Optional.of(conv));
        when(participantRepo.existsByConversationAndUser(conv, alice))
                .thenReturn(true);
        ...
        when(participantRepo.findByConversation(conv))
                .thenReturn(List.of(aliceP, bobP));
        ...
        service.sendMessage(
                ...
                alice.getPublicId().toString()
        );

        verify(messageRepo).save(any(Message.class));

        verify(broker).convertAndSend(
                eq("/topic/conversation." + conv.getPublicId()),
                any(Object.class)
        );

        verify(broker).convertAndSendToUser(
                eq(alice.getPublicId().toString()),
                eq("/queue/messages"),
                any(Object.class)
        );

        verify(broker).convertAndSendToUser(
                eq(bob.getPublicId().toString()),
                eq("/queue/messages"),
                any(Object.class)
        );
    ...
    void sendMessage_rejects_whenSenderIsNotParticipant() {
        when(userRepo.findByPublicId(intruder.getPublicId()))
                .thenReturn(Optional.of(intruder));
        when(conversationRepo.findByPublicId(conv.getPublicId()))
                .thenReturn(Optional.of(conv));
        when(participantRepo.existsByConversationAndUser(conv, intruder))
                .thenReturn(false);
    ...
    void getOrCreateDirectConversation_returnsExisting_whenAlreadyPresent() {
        when(userRepo.findByPublicId(alice.getPublicId()))
                .thenReturn(Optional.of(alice));
        when(userRepo.findByPublicId(bob.getPublicId()))
                .thenReturn(Optional.of(bob));
        when(conversationRepo.findDirectBetween(alice, bob))
                .thenReturn(Optional.of(conv));

        ConversationParticipant aliceP = new ConversationParticipant();
        aliceP.setConversation(conv);
        aliceP.setUser(alice);
        aliceP.setLastReadAt(null); // triggers correct branch
        ...
        when(participantRepo.findByConversationAndUser(conv, alice))
                .thenReturn(Optional.of(aliceP));

        when(messageRepo.findFirstByConversationOrderByCreatedAtDesc(conv))
                .thenReturn(Optional.empty());

        when(messageRepo.countByConversationAndSenderNot(conv, alice))
                .thenReturn(0L);
