...
public class RepositoryAccessService {
    ...
    public void ensureOwnerAccess(RepositoryEntity repository) {
        String owner = repository.getOwner();
        if (owner == null) throw new ForbiddenException("Repository owner is not set");
        
        Set<String> actorKeys = getCurrentActorKeys();
        String slugifiedOwner = owner.toLowerCase().replace(" ", "-");
        
        if (actorKeys.contains(owner.toLowerCase()) || actorKeys.contains(slugifiedOwner)) {
            return;
        }
    ...
    public void revokeAccess(String owner, String name, String actorKey) {
        log.info(">>> Revoking access for repository {}/{} from actor: {}", owner, name, actorKey);
        RepositoryEntity repository = getRepositoryOrThrow(owner, name);
        ensureWriteAccess(repository);
        ...
        log.info(">>> Access revoked successfully for {}/{} (actor: {})", owner, name, actorKey);
    }
            ...
            try {
                ...
                userRepository.findByPublicId(id).ifPresent(user -> {
                    ...
                    if (user.getFirstName() != null && user.getLastName() != null) {
                        String fullName = (user.getFirstName() + " " + user.getLastName()).trim().toLowerCase();
                        keys.add(fullName.replace(" ", "-"));
                        keys.add(fullName);
                    }
                });
        ...
        if (auth != null && auth.isAuthenticated() && !"anonymousUser".equals(auth.getPrincipal())) {
            try {
                ...
                if (user.isPresent() && user.get().getProfile() != null) {
                    return user.get().getProfile().getUsername().toLowerCase().replaceAll(" ", "-");
                }
            } catch (Exception ignored) {}
        }
        return getCurrentDisplayName().toLowerCase().replaceAll(" ", "-");
    }
