...
public class GitStorageService {
    ...
    private static final Logger log = LoggerFactory.getLogger(GitStorageService.class);
    ...
    public void renameOwnerDirectory(String oldOwner, String newOwner) {
        Path sourcePath = basePath.resolve(oldOwner).normalize();
        Path destinationPath = basePath.resolve(newOwner).normalize();

        if (!Files.exists(sourcePath)) {
            log.warn("Owner directory does not exist, skipping rename: {}", oldOwner);
            return;
        }
        if (Files.exists(destinationPath)) {
            log.warn("Destination owner directory already exists: {}", newOwner);
            // Optionally merge or handle this case. For now, we skip.
            return;
        }

        try {
            Files.move(sourcePath, destinationPath, StandardCopyOption.ATOMIC_MOVE);
            log.info("Successfully renamed owner directory from {} to {}", oldOwner, newOwner);
        } catch (IOException e) {
            log.error("Failed to rename owner directory: {}", e.getMessage(), e);
            throw new RuntimeException("Failed to rename owner directory on disk", e);
        }
    }
}
