...
public class GitAnalysisService {
    ...
    public List<String> getBranches(String owner, String name) {
        ...
        String repoPath = owner + "/" + name + (name.endsWith(".git") ? "" : ".git");
        File repoDir = gitStorageService.getBasePath().resolve(repoPath).toFile();
        
        log.info("Listing branches for repository at: {}", repoDir.getAbsolutePath());
        ...
        try (Repository repository = openRepository(owner, name);
            ...
            // Fallback pour les bare repositories si branchList() est vide
            if (branches.isEmpty()) {
            ...
            // Fallback ultra-robuste : si JGit ne voit rien, on regarde physiquement le dossier refs/heads
            if (branches.isEmpty()) {
                File refsHeads = new File(repoDir, "refs/heads");
                if (refsHeads.exists() && refsHeads.isDirectory()) {
                    File[] files = refsHeads.listFiles();
                    if (files != null) {
                        for (File f : files) {
                            if (f.isFile()) branches.add(f.getName());
                        }
                    }
                }
            }
    ...
    public List<RepositoryFileEntity> getFiles(String owner, String name, String branch, String path) {
        List<RepositoryFileEntity> files = new ArrayList<>();

        try (Repository repository = openRepository(owner, name)) {
            ObjectId lastCommitId = repository.resolve(branch);
            
            // Fallback: If 'main' was requested but doesn't exist, try 'master', then any branch
            if (lastCommitId == null && (branch == null || branch.equals("main") || branch.equals("master"))) {
                lastCommitId = repository.resolve("refs/heads/master");
                if (lastCommitId == null) {
                    lastCommitId = repository.resolve("master");
                }
                if (lastCommitId == null) {
                    lastCommitId = repository.resolve("refs/heads/main");
                }
            }
            
            if (lastCommitId == null) {
                // Still null? Try the first branch available
                List<String> allBranches = getBranches(owner, name);
                if (!allBranches.isEmpty()) {
                    for (String b : allBranches) {
                        if (!b.equals("main")) {
                            lastCommitId = repository.resolve(b);
                            if (lastCommitId != null) break;
                        }
                    }
                }
            }
                ...
                try (TreeWalk treeWalk = new TreeWalk(repository)) {
                    if (path != null && !path.isBlank()) {
                        try (TreeWalk pathWalk = TreeWalk.forPath(repository, path, tree)) {
                            if (pathWalk != null && (pathWalk.getFileMode(0).getBits() & 0040000) != 0) { // Check if it's a directory
            ...
            log.error("Error listing files for {}/{} branch {} path {}: {}", owner, name, branch, path, e.getMessage());
        }
    ...
    public String getFileContent(String owner, String name, String branch, String filePath) {
        try (Repository repository = openRepository(owner, name)) {
            ...
            if (lastCommitId == null && (branch == null || branch.equals("main") || branch.equals("master"))) {
                lastCommitId = repository.resolve("refs/heads/master");
                if (lastCommitId == null) lastCommitId = repository.resolve("master");
                if (lastCommitId == null) lastCommitId = repository.resolve("refs/heads/main");
            }
            if (lastCommitId == null) {
                List<String> allBranches = getBranches(owner, name);
                if (!allBranches.isEmpty()) {
    ...
    public List<RepositoryCommitEntity> getCommits(String owner, String name, String branch) {
        ...
        try (Repository repository = openRepository(owner, name);
            ...
            if (branchId == null && (branch == null || branch.equals("main") || branch.equals("master"))) {
                branchId = repository.resolve("refs/heads/master");
                if (branchId == null) branchId = repository.resolve("master");
                if (branchId == null) branchId = repository.resolve("refs/heads/main");
            }
            if (branchId == null) {
                List<String> allBranches = getBranches(owner, name);
                if (!allBranches.isEmpty()) {
                    for (String b : allBranches) {
                        if (!b.equals("main")) {
                            branchId = repository.resolve(b);
                            if (branchId != null) break;
                        }
                    }
                }
            }
            ...
            Iterable<RevCommit> commitsLog = git.log()
                    .add(branchId)
                    .call();
            ...
            for (RevCommit commit : commitsLog) {
                ...
                commitEntity.setDate(Date.from(commit.getAuthorIdent().getWhenAsInstant()).toString());
    ...
    private void populateCommitDiff(
            Repository repository,
            RevCommit commit,
            RepositoryCommitEntity target
