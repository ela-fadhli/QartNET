...
public class RepositoryController {
    ...
    public ResponseEntity<Void> deleteRepository(@PathVariable Long id) {
        ...
        try {
            RepositoryEntity repository = repositoryService.getById(id)
                    .orElseThrow(() -> new tn.enicarthage.qartnet.shared.exception.ResourceNotFoundException("Repository not found"));
            repositoryAccessService.ensureOwnerAccess(repository);
            
            repositoryService.deleteRepository(id);
            log.info(">>> Repository {} deleted successfully", id);
            return ResponseEntity.noContent().build();
        } catch (tn.enicarthage.qartnet.shared.exception.ForbiddenException e) {
            log.error(">>> Forbidden to delete repository {}: {}", id, e.getMessage());
    ...
    public ResponseEntity<RepositoryEntity> updateRepository(
        ...
        if (request.getVisibility() != null && request.getVisibility() != repository.getVisibility()) {
            repositoryAccessService.ensureOwnerAccess(repository);
        }
    ...
    @PutMapping("/{owner}/{name}/default-branch")
    public ResponseEntity<RepositoryEntity> setDefaultBranch(
            @PathVariable String owner,
            @PathVariable String name,
        ...
        RepositoryEntity repository = repositoryAccessService.getRepositoryOrThrow(owner, name);
        repositoryAccessService.ensureWriteAccess(repository);
        return ResponseEntity.ok(repositoryService.setDefaultBranch(owner, name, branch));
    }
